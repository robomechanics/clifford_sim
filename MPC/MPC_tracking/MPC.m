...
for i = 2:mpc_n
    ...
    % update mpc matrix
    index_x_next = (i-1) * DIM_X+1 : i*DIM_X;
    index_x_prev = (i-2) * DIM_X + 1:(i-1) * DIM_X;
    index_u_next = (i-1) * DIM_U+1:i * DIM_U;
    Y_i = (i-1) * DIM_Y + 1:i * DIM_Y;
    
    A_bar(index_x_next, :) = Ad * A_bar(index_x_prev, :);
    
    for j = 1:i-1
        idx_u_j = (j-1) * DIM_U + 1:j * DIM_U;
        B_bar(index_x_next, idx_u_j) = Ad * B_bar(index_x_prev, idx_u_j);
    end
    
    B_bar(index_x_next, index_u_next) = Bd;
    W_bar(index_x_next) = Ad * W_bar(index_x_prev) + wd;
    C_bar(Y_i, index_x_next) = Cd;
    Q_bar(Y_i, Y_i) = Q;
    R_bar(index_u_next, index_u_next) = R;
