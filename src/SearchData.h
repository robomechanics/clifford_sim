...
public:
  ...
  SearchData(int maxStates, float _robotLength, float _maxSteeringAng){//, ros::Publisher* _path_pub){
  ...
  SearchData(float _robotLength, float _maxSteeringAng){//, ros::Publisher* _path_pub){
  ...
  std::vector<coord> getSuccessorCoords(coord robotCoord)
  {
    ...
    for (int i=0;i<gridConnectX.size();i++)
    {
      ...
      float turnRadius = dirX_r/sin(abs(headingChange));
