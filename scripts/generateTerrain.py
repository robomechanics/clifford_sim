...
def GenerateSTL(GAME_WIDTH,heightScale,filename):
    coordinates = DiamondSquare(GAME_WIDTH) #The size has to be 2^n+1
    mapHeights = np.zeros((GAME_WIDTH,GAME_WIDTH))
    rowCounter = 0
    columnCounter = 0
    for item in coordinates:
        mapHeights[rowCounter,columnCounter] = item[2];
        columnCounter = columnCounter+1
        if columnCounter == GAME_WIDTH:
            columnCounter = 0
